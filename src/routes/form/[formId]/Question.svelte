<script lang="ts">
    ...
    let {question = $bindable(), deleteMe}: { question: Question, deleteMe: () => void} = $props();
    ...
    $effect(() => {
        ...
        untrack(() => {
            if(question.type === "range" && (Array.isArray(question.answers) || question.answers == null)) {
                question.answers = {min: 1, max: 100, step: 5} as RangeSettings;
...
<div class="text-2xl text-gray-200 p-4 flex flex-col relative">
    <div class="flex flex-row">
        <input bind:value={question.content} class="input-stealth my-2" placeholder="Question" autocomplete="off">
        <select bind:value={question.type} class="button" name="questionType" required autocomplete="">
    ...
    <label for={`required-${question.id}`}>
        <span>Required</span>
        <input type="checkbox" bind:checked={question.required} id={`required=${question.id}`}>
    </label>
            ...
            {#each question.answers as answer (answer.id)}
                <div class="flex flex-row w-full justify-between">
                    <label for="new-choice-{answer}"
                        ...
                        <input type="text" bind:value={answer.content} placeholder={`Option ${answer.id}`} class={`answerInput-${question.id}`} autocomplete="off">
                    </label>
